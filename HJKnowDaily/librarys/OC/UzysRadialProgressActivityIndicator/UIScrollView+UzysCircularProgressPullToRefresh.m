...
- (void)addPullToRefreshActionHandler:(actionHandler)handler navigationController:(UINavigationController *)navigationController
{
    if(self.pullToRefreshView == nil)
    {
        [self addPullToRefreshActionHandler:handler portraitContentInsetTop:CGFLOAT_MAX landscapeInsetTop:CGFLOAT_MAX navigationController:navigationController];
    }
}


- (void)addPullToRefreshActionHandler:(actionHandler)handler portraitContentInsetTop:(CGFloat)pInsetTop landscapeInsetTop:(CGFloat)lInsetTop navigationController:(UINavigationController *)navigationController
{
    if(self.pullToRefreshView == nil)
    {
        ...
        view.frame = CGRectMake((self.bounds.size.width - view.bounds.size.width)/2,
                                -view.bounds.size.height + 100, view.bounds.size.width, view.bounds.size.height);
        ...
        NSLog(@"originalTopInset:%lf",view.originalTopInset);

        // 这里 注意偏移量的变化 ios 开发都是坑  友木有。
